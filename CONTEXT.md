...
- **Managed Field** — a Field a Feature contributed rather than the writer declaring it. The system owns its value and the writer does not set it. Declaring a Field whose key a Feature also provides is a config error, not a silent winner.
- **Publication State** — whether a Collection Item is published, recorded in its committed Data. Not to be confused with a **Draft**: a Draft is Kobun's working copy of content and never reaches the repository, whereas Publication State lives in the Source alongside the content it describes. Kobun never guesses one it did not write — for content it did not author, the absence of the key is itself a state, and what that state means is the site's convention.
