...
- e97c764: Three fixes: a slug field can now sit inside an object field and fills in from a text field beside it as you type; a document field inside an object or array field, or a slug field in an array, is now flagged as a config error instead of breaking the editor; and after each release, every project's config is checked again once, so a config that was accepted before is not kept after the rules change.

## 0.6.1

### Patch Changes

- c78af01: Four fixes: dates and times now follow your own locale and clock instead of always US English, and say which time zone they are read on; the sidebar stays collapsed once you collapse it; the dashboard shows your most recent drafts with a link to the rest, and tells you when you have none; and a project switcher row is now the link itself, so the whole row is clickable.

## 0.6.0

### Minor Changes

- dad87f4: A settings area: account settings with profile, preferences, sessions and account deletion, a project settings page showing what your config declares, disconnecting a project gated on the drafts it would destroy, and the sidebar footer turned into your account menu.

### Patch Changes

- 04b57aa: The Save button's choice now follows you, not your browser. Whichever of Save and Save to GitHub you set it to is remembered against your account, so it is the same on your laptop, on your other browser, and after you clear your cookies. It used to be a cookie, which meant a choice you made at your desk was not the one waiting for you anywhere else.

  One thing to know: because the old choice was a cookie, this release cannot carry it across. If you had set the button to Save to GitHub, it is Save again the first time you open an editor — set it once more and it stays for good.
