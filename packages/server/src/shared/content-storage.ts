...
export abstract class BaseContentStorage {
	...
	protected async parseFileContent(
		...
		if (!result.success) {
			throw new ValidationError('Failed to validate content metadata', {
				filePath,
 ...
 */
export class CollectionContentManager
	extends BaseContentStorage
	implements CollectionOperations
{
	...
	async readItem({
		...
		slug,
		schema,
	}: { id?: string; slug?: string; schema: z.ZodType }): Promise<
	...
	> {
		...
		if (!targetSlug && id) {
			...
			// biome-ignore lint/suspicious/noExplicitAny: item structure varies by schema configuration
	...
	async writeItem({
		...
		content,
		oldSlug,
	}: { slug: string; content: string; oldSlug?: string }): Promise<void> {
 ...
 */
export class SingletonContentManager
	extends BaseContentStorage
	implements SingletonOperations
{
	constructor(
		protected storage: ContentStorage,
