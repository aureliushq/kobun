...
> **Amended.** The **`normalizeMetadata`** half of the Consequences below is superseded by [ADR-0009](./0009-one-yaml-implementation-behind-every-format.md). Normalization existed only to undo js-yaml turning `md`/`mdx` frontmatter scalars into `Date` objects, and it could not undo it without guessing what the writer had typed — the guess is what [#114](https://github.com/aureliushq/kobun/issues/114) and [#115](https://github.com/aureliushq/kobun/issues/115) report. Every Format now shares one YAML implementation, which emits no `Date` at all, so `normalizeMetadata` is deleted and `parseDocument` returns what the parser gave it. The module owns `canonicalMetadata` alone. Everything else here stands as accepted.
